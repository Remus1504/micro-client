...
import { useGetStudentByUsernameQuery } from "src/features/student/services/student.service";
...
import OfferModal from "src/shared/modals/EnrolmentOffer";
import {
  checkFile,
  fileType,
  readAsBase64,
...
import { firstLetterUppercase, showErrorToast } from "src/shared/utils/utils";
...
const ChatWindow: FC<IChatWindowProps> = ({
  ...
  const receiverRef = useRef<StudentDocument>();
  ...
  const { data: studentData, isSuccess: isStudentSuccess } =
    useGetStudentByUsernameQuery(`${firstLetterUppercase(`${username}`)}`);
  const { data } = useGetCourseByIdQuery(
    singleMessageRef.current
      ? `${singleMessageRef.current?.courseId}`
      : NOT_EXISTING_ID
  );
  const [saveChatMessage] = useSaveChatMessageMutation();

  if (isStudentSuccess) {
    receiverRef.current = studentData.student;
  }
  ...
  const sendMessage = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      const messageBody: IMessageDocument = {
        ...
        body: message,
        courseId: singleMessageRef?.current?.courseId,
        instructorId: singleMessageRef?.current?.instructorId,
        studentId: singleMessageRef?.current?.studentId,
    ...
    <>
      {!isLoading && displayCustomOffer && (
        <OfferModal
          ...
          courseTitle={data && data?.course?.title ? data?.course?.title : ""}
              ...
              <div className="bottom-0 left-0 right-0 z-0 h-28 px-4 ">
                ...
                <div className="flex cursor-pointer flex-row justify-between">
                  <div className="flex gap-4">
                    ...
                    {!showImagePreview &&
                      singleMessageRef.current &&
                      singleMessageRef.current.instructorId ===
                        instructor?._id && (
                        <Button

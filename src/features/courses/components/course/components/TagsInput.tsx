...
import { ITagsInputProps } from "src/features/courses/interfaces/course.interface";
...
const TagsInput: FC<ITagsInputProps> = (props): ReactElement => {
  ...
  const onKeyDown = (
    ...
    if (tagsList.length + 1 <= maxTagCount) {
      if (
        ...
        setItemInput("");
        const courseInfoList: string[] = courseInfo[`${itemName}`] as string[];
        setCourseInfo({
          ...courseInfo,
          [`${itemName}`]: [...courseInfoList, trimmedInput],
        });
      }
    }
    ...
    if (
      ...
      setCourseInfo({ ...courseInfo, [`${itemName}`]: [...tagsCopy] });
    }
  ...
  const deleteTag = (index: number): void => {
    ...
    setCourseInfo({
      ...courseInfo,
      [`${itemName}`]: courseInfoList.filter((_, i: number) => i !== index),
    });
  };

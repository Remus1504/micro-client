...
import {
  ...
  ArrayOfSkillsSchema,
  personalInfoSchema,
} from "../schemes/instructor.schema";

interface IUseInstructorSchema {
...
const useInstructorSchema = ({
  personalInfo,
  experienceFields,
  educationFields,
  skillsFields,
  languageFields,
}: IUseInstructorSchema): [
  () => Promise<boolean>,
  IPersonalInfoData[],
  IExperience[],
  IEducation[],
  ...
  async function schemaValidation(): Promise<boolean> {
    ...
    const isPersonalInfoValid = await personalInfoSchema.isValid(personalInfo, {
      abortEarly: false,
    });

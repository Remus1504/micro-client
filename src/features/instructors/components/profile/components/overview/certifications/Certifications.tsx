...
import { ICertificate } from "src/features/instructors/interfaces/instructor.interface";
...
const Certifications: FC = (): ReactElement => {
  ...
  const { instructorProfile, showEditIcons } = useContext(instructorContext);
  ...
  return (
    <div className="border-grey mt-6 border bg-white">
      ...
      <ul className="mb-0 list-none pt-1.5">
        ...
        {!showCertificateAddForm && (
          <>
            {instructorProfile?.certificates.map(
              (certificate: ICertificate) => (
                <li key={uuidv4()} className="mb-2 flex justify-between">
                  ...
                  {showCertificateEditForm &&
                    selectedCertificate?.name === certificate.name && (
                      <CertificateEditFields
                        type="edit"
        ...
        {instructorProfile.certificates.length === 0 &&
          !showCertificateAddForm &&
          !showCertificateEditForm && (

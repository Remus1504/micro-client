...
const Description: FC = (): ReactElement => {
  const { instructorProfile, setInstructorProfile, showEditIcons } =
    useContext(instructorContext);
  const [showDescriptionEditForm, setShowDescriptionEditForm] =
    useState<boolean>(false);
  const [description, setDescription] = useState<string>(
    instructorProfile.description ? `${instructorProfile.description}` : ""
  );
          ...
          <div className="flex w-full flex-col">
            ...
            <div className="mx-3 mb-2 flex cursor-pointer justify-start">
              <Button
                ...
                label="Update"
                onClick={() => {
                  if (setInstructorProfile) {
                    setInstructorProfile({ ...instructorProfile, description });

...
import TextInput from "../Entries/Input";
import { expectedCourseDuration, showErrorToast } from "../utils/utils";
import { IModalProps } from "./interfaces/modal.interface";
import ModalBg from "./ModalBg";

interface InstructorOffer {
  description: string;
  price: string;
  duration: string;
  startDate: string;
...
const OfferModal: FC<IModalProps> = ({
  ...
  const [offer, setOffer] = useState<InstructorOffer>({
    description: "",
    price: "",
  ...
  const sendCourseOffer = async (): Promise<void> => {
    try {
      const messageBody: IMessageDocument = {
        conversationId: `${singleMessage?.conversationId}`,
        hasConversationId: true,
        body: "Here's your custom offer",
        courseId: singleMessage?.courseId,
        instructorId: singleMessage?.instructorId,
        studentId: singleMessage?.studentId,
        ...
        offer: {
          ...
          durationInDays: parseInt(offer.duration),
          oldStartDate: offer.startDate,
          newStartDate: offer.startDate,
          accepted: false,
          cancelled: false,
        },
      };
            ...
            <div className="mb-6">
              ...
              <div id="country" className="relative mb-5 mt-2">
                <Dropdown
                  ...
                  onClick={(item: string) => {
                    const durationInDays: number = parseInt(item);
          ...
          <div className="px-5 py-4">
            <div className="ml-2 flex w-full justify-center text-sm font-medium">
              <Button
                ...
                disabled={!offer.description || !offer.price || !offer.duration}
                label="Send Offer"
                onClick={sendCourseOffer}
              />
            </div>
          </div>
        </div>

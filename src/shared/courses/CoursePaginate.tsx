...
const CoursePaginate: FC<ICoursePaginateProps> = ({
  courses,
  totalCourses,
  showNumbers,
  itemsPerPage,
  ...
  const paginationCount: number[] = [
    ...Array(Math.ceil((totalCourses as number) / itemsPerPage)).keys(),
  ];
      ...
      <ul className="flex gap-8">
        <div
          ...
          onClick={() => {
            if (itemOffset - 1 > 0) {
              ...
              const firstItem: InstructorCourse = courses[0];
        ...
        <div
          ...
          onClick={() => {
            if (itemOffset + 1 <= paginationCount.length) {
              itemOffset += 1;
              setPaginationType("forward");
              const lastItem: InstructorCourse = courses[courses.length - 1];
        ...
        >
          <FaArrowRight
            ...
            color={`${
              itemOffset === paginationCount.length ? "grey" : "black"
            }`}
          />
        </div>
      </ul>

...
import CourseIndexItem from "src/features/index/courses-tabs/CourseIndexItem";
...
const TopCoursesView: FC<ICourseTopProps> = ({
  courses,
  title,
  subTitle,
  category,
          ...
          {courses.map((course: InstructorCourse) => (
            <div key={uuidv4()} className={`${width}`}>
              {type === "home" ? (
                ...
                <CourseIndexItem course={course} />
              )}
            </div>
          ))}
        </div>
        {!scroll.end && courses.length > 2 && (
          <span
            onClick={slideRight}

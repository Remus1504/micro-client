...
import { logout } from 'src/features/auth/reducers/logout.reducers';
import { authApi } from 'src/features/auth/services/authentication.service';
...
export const expectedCourseDuration = (): string[] => {
  return [
    '1 Day Duration',
    '2 Days Duration',
    '3 Days Duration',
...
export const enrolmentTypes = (
  ...
  const enrolmentList: IEnrolmentDocument[] = filter(
    enrolments,
    (enrolment: IEnrolmentDocument) =>
      lowerCase(enrolment.status) === lowerCase(status),
  );
  return enrolmentList.length;
};

export const instructorOrderList = (
  status: string,
  enrolments: IEnrolmentDocument[],
): IEnrolmentDocument[] => {
  const enrolmentList: IEnrolmentDocument[] = filter(
